...
drone_transform = A.Compose([
    ...
    A.RandomResizedCrop(size=(640, 640), scale=(0.5, 1.0), p=0.4), # 🛠 V4: 다양한 스케일 학습 추가 (Albumentations 1.4+ 호환)
    A.ShiftScaleRotate(
        shift_limit_x=0.05,
        shift_limit_y=0.1,    # 🛠 V4: 수직 이동 소폭 하향 조정
        scale_limit=0.1,
        rotate_limit=15,       # 🛠 V4: 회전 각도 20 -> 15 (왜곡 최소화)
        border_mode=cv2.BORDER_CONSTANT,
        p=0.5
    ),
    A.OneOf([
        A.GridDistortion(distort_limit=0.05, p=1.0),
        # ❌ OpticalDistortion/ElasticTransform 제거 (정밀도 저하 원인)
    ], p=0.1), # 🛠 V4: 기하 왜곡 확률 하향 (0.2 -> 0.1)
    
    # 💨 2. 촬영 환경 및 가려짐 (Camera Effects & Occlusion)
    A.OneOf([
        A.MotionBlur(blur_limit=(3, 5), p=1.0),
        A.GaussianBlur(blur_limit=(3, 5), p=1.0),
    ], p=0.15), # 🛠 V4: 블러 확률 하향 (0.2 -> 0.15) - 선명도 유지
    A.CoarseDropout(max_holes=6, max_height=16, max_width=16, p=0.1),

    # ☀️ 3. 조명 변화 및 노출 (Lighting & Exposure)
    A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.4),
    A.RandomGamma(gamma_limit=(80, 120), p=0.3),
    A.HueSaturationValue(hue_shift_limit=10, sat_shift_limit=20, val_shift_limit=10, p=0.1), # 🛠 V4: 색상 변화 완화
    A.ISONoise(color_shift=(0.01, 0.02), intensity=(0.1, 0.3), p=0.2),

    # 🩹 4. 미세 균열 가시성 강화 (Fixed Strategy)
    A.OneOf([
        A.Sharpen(alpha=(0.2, 0.4), p=1.0), # 🛠 V4: 정밀도 위해 샤픈 강화
        A.CLAHE(clip_limit=3.0, p=1.0),    # 🛠 V4: 대비 강화 소폭 상승
    ], p=0.5), # 🛠 V4: 가시성 강화 확률 상승 (0.4 -> 0.5)
...
def process_dataset(input_root, output_root, val_ratio=0.2):
    ...
    # 🛠 V4: Background Capping (결함 대비 20%로 증계 - Precision 강화)
    random.seed(42)
    max_bg_count = int(len(defect_imgs) * 0.2)
    if len(bg_imgs) > max_bg_count:
        print(f"V4 Pipeline: Reducing backgrounds from {len(bg_imgs)} to {max_bg_count} (20% of defects)")
    ...
    print(f"V4 Pipeline: Total({len(all_files)}), Defect({len(defect_imgs)}), BG({len(bg_imgs)})")
...
if __name__ == "__main__":
    ...
    INPUT_DIR = BASE_DIR / "datasets" / "defect_review.v10i.yolov8"
