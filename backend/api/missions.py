...
# 특정 미션의 모든 감지 결과(크랙 위치 등)를 가져오는 API
@router.get("/{mission_id}/detections", response_model=List[schemas.Detection])
def get_mission_detections(mission_id: int, db: Session = Depends(get_db)):
    # DB에서 해당 미션의 감지 결과만 다 긁어모읍니다.
    detections = db.query(models.Detection).filter(models.Detection.mission_id == mission_id).all()
    return detections

@router.get("/api/config/google-maps")
async def get_google_maps_key():
    # 환경 변수에서 키를 가져와 프론트엔드에 전달합니다.
