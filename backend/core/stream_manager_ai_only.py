...
class StreamManager:
    def __init__(self):
        ...
        model_path = os.path.join(project_root, "runs", "detect", "crack-with-background", "best.pt")
    ...
    def _inference_loop(self):
        ...
        while self.is_running:
            ...
            results = self.model.track(frame_to_infer, conf=0.6, persist=True, verbose=False)
            inference_count += 1
                                ...
                                if conf > 0.7:
                                    ...
                                    if crop_img.size > 0:
                                        ...
                                        if embedding is not None:
                                            ...
                                            is_dup, max_sim = self.reid_manager.is_duplicate(self.active_mission_id, embedding)
                                            
                                            if is_dup:
                                                logger.info(f"🔄 [Re-ID Drop] Duplicate crack detected. Sim: {max_sim:.2f} >= Threshold")
                                            else:
